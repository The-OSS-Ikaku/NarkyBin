...
//				text += "<br>All the changelings are dead! Restart in [round((changeling.TIME_TO_GET_REVIVED-(world.time-changeling.changelingdeathtime))/10)] seconds."
	...
	Topic(href, href_list)
		if(!check_rights(R_TRAITOR)) return
		...
		if (href_list["role_edit"])
			...
			var/new_obj_type = input("Select objective type:", "Objective type", def_value) as null|anything in list("assassinate", "vore", "vore_digest", "digest", "get_digested", "debrain", "protect", "prevent", "hijack", "escape", "vore_escape", "vore_opt_escape", "survive", "steal", "download", "nuclear", "capture", "absorb", "custom")
			if (!new_obj_type) return

			var/datum/objective/new_objective = null

			switch (new_obj_type)
				if ("assassinate","protect","debrain","vore","vore_digest","digest","get_digested")
				...
				if ("vore_escape")
					new_objective = new /datum/objective/vore_escape
					new_objective.owner = src

				if ("vore_opt_escape")
					new_objective = new /datum/objective/vore_opt_escape
					new_objective.owner = src
