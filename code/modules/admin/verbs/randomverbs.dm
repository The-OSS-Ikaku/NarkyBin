...
/client/proc/cmd_admin_delete(atom/O as obj|mob|turf in world)
	...
	if (alert(src, "Are you sure you want to delete:\n[O]\nat ([O.x], [O.y], [O.z])?", "Confirmation", "Yes", "No") == "Yes")
		if(istype(O,/mob))
			var/mob/M=O
			if(M.client&&M.client.holder) //are they an admin?
				if(!check_rights(R_SPECIAL,0)) //are we an upper admin?
					message_admins("[key_name_admin(usr)] has attempted to delete [key_name(M)]")
					log_admin("[key_name_admin(usr)] has attempted to delete [key_name(M)]")
					return //no deleting admins
...
/client/proc/cmd_admin_gib(mob/M as mob in mob_list)
	...
	if(M.client&&M.client.holder) //are they an admin?
		if(check_rights(R_SPECIAL,0)) //are we an upper admin?
			message_admins("[key_name_admin(usr)] has attempted to gib [key_name(M)]")
			log_admin("[key_name_admin(usr)] has attempted to gib [key_name(M)]")
			return //no gibbing upper admins
