...
datum/preferences
	...
	var/ckey = "null" //tempfix for stuff, doesn't hurt anything so can be kept in forever
	var/path
...
/datum/preferences
		...
		switch(current_tab)
			if (0) // Character Settings#
				...
				if(config.mutant_races)
					...
					dat += "<b>Mutant Race:</b><a href='?_src_=prefs;preference=mutant_race;task=input'>[mutant_race]</a><BR>"
					...
					if(!kpcode_cantaur(mutant_race))
						dat += " (not available for [mutant_race])"
					dat += "<BR>"
					if(special_color[1])
					...
					if(special_color[3])
						dat += "<span style='border:1px solid #161616; background-color: #[special_color[3]];'>&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=special_color;which=3;task=input'>Secondary</a><BR>"
					else
			...
			if (1) // Game Preferences
				...
				if(jobban_isbanned(user, "Syndicate"))
					dat += "<b>You are unable to choose from antagonist roles.</b>"
					src.be_special = 0
				else
					var/n = 0
		...
		switch(href_list["task"])
			...
			if("input")
				switch(href_list["preference"])
					...
					if("mutant_race")
						var/new_mutant_race = input(user, "Choose your character's mutant race:", "Character Preference")  as null|anything in kpcode_race_getlist(ckey)
						if(new_mutant_race)
					...
					/*if("special_color_one")
						switch(alert("Use a special primary colour?","Character Preference","Yes","No","Cancel"))
							if("Yes")
								var/new_color = input(user, "Choose your character's primary colour:", "Character Preference") as null|color
								if(new_color)
									special_color_one = sanitize_hexcolor(new_color)
							if("No")
								special_color_one=null

					if("special_color_two")
						switch(alert("Use a special secondary colour?","Character Preference","Yes","No","Cancel"))
							if("Yes")
								var/new_color = input(user, "Choose your character's secondary colour:", "Character Preference") as null|color
								if(new_color)
									special_color_two = sanitize_hexcolor(new_color)
							if("No")
								special_color_two=null

					if("character_size")
						character_size = character_size=="normal" ? "small" : "normal"*/
					...
					if("character_size")
						var/new_size = input(user, "Choose your character's size:", "Character Preference")  in list("normal","small","big")
						if(new_size)
							if(new_size=="big" && !is_whitelisted(ckey))
	...
	proc/copy_to(mob/living/carbon/human/character)
		...
		if(character.dna)
			...
			//character.dna.special_color_one = special_color_one
			//character.dna.special_color_two = special_color_two
