...
/datum/dna/proc/generate_race_block()
	...
	if(kpcode_race_list[mutantrace])
		block_gen = construct_block(kpcode_race_list.Find(mutantrace), kpcode_race_list.len+1)
	else
		block_gen = construct_block(kpcode_race_list.len+1, kpcode_race_list.len+1)
	return block_gen
