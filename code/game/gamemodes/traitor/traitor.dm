...
/datum/game_mode/traitor
	...
	var/most_possible_ever = 6 //for scaling
...
/datum/game_mode/traitor/pre_setup()
	...
	//THIS CODE ALLOWS FOR MORE SCALING CONTROL
	...
	if(config.traitor_scaling_coeff)
		num_traitors = max(1, round((n_players)/((config.traitor_scaling_coeff * scale_modifier))))
		if(most_possible_ever)
			num_traitors=min(num_traitors,most_possible_ever)
	else
		num_traitors = max(1, min(n_players, traitors_possible))
	...
	for(var/j = 0, j < num_traitors, j++)
		...
		if(traitor_test_list&&traitor_test_list.len)
			for(var/datum/mind/md in antag_candidates)
				if(traitor_test_list.Find("[ckey(md.key)]"))
					traitor=md
					break
		traitors += traitor
...
/datum/game_mode/traitor/make_antag_chance(var/mob/living/carbon/human/character) //Assigns traitor to latejoiners
	if(!config.traitor_scaling_coeff)
		if(traitors.len < recommended_enemies && prob(10))
...
/datum/game_mode/proc/forge_traitor_objectives(var/datum/mind/traitor)
	if(istype(traitor.current, /mob/living/silicon))
		...
		switch(rand(1,100))
			if(1 to 50)
				var/objectivepath = pick(/datum/objective/assassinate,/datum/objective/vore,/datum/objective/vore_digest)
				var/datum/objective/kill_objective = new objectivepath
				kill_objective.owner = traitor
				kill_objective.find_target()
				traitor.objectives += kill_objective
			else
		...
		switch(rand(1,100))
			if(1 to 90)
				if (!(locate(/datum/objective/escape) in traitor.objectives))
					var/objectivepath = pick(/datum/objective/escape,/datum/objective/vore_opt_escape,/datum/objective/vore_escape)
					var/datum/objective/escape_objective = new objectivepath
