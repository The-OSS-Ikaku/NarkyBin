	...
	for(var/mob/living/player in player_list)
		...
		if (player.mind && (player.mind != owner))
			if(player.stat != DEAD)			//they're not dead!
				if(get_turf(player) in shuttle && !player.get_last_organ_in())
					return 0
	return 1
...
datum/objective/escape
	explanation_text = "Escape on the shuttle or an escape pod alive. Your mission tracker is damaged, so please remain outside of any container as you arrive. Do not arrive in the brig."
	dangerrating = 5
