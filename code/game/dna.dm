...
/datum/dna/proc/generate_uni_identity(mob/living/carbon/character)
	...
	if(istype(character))
		...
		if(istype(character, /mob/living/carbon/human))
			...
			/*if(kpcode_race_list[mutantrace])
				L[DNA_MUTANTRACE_BLOCK] = construct_block(kpcode_race_list.Find(mutantrace), kpcode_race_list.len+1)
			else
				L[DNA_MUTANTRACE_BLOCK] = construct_block(kpcode_race_list.len+1, kpcode_race_list.len+1)*/
	...
	if(!L[DNA_MUTANTRACE_BLOCK])
		if(prob(60))
			L[DNA_MUTANTRACE_BLOCK] = construct_block(rand(1,14), kpcode_race_list.len+1)
		else
			L[DNA_MUTANTRACE_BLOCK] = construct_block(1, kpcode_race_list.len+1)
...
/proc/updateappearance(mob/living/carbon/C)
	...
	if(istype(C, /mob/living/carbon/human))
		...
		var/mutantrace_c = deconstruct_block(getblock(structure, DNA_MUTANTRACE_BLOCK), kpcode_race_list.len+1)
		if(mutantrace_c<=kpcode_race_list.len && kpcode_race_restricted(kpcode_race_list[mutantrace_c])!=2)
			C.dna.mutantrace=kpcode_race_list[mutantrace_c]
